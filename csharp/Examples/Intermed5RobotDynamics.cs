        ...
        public void Run(string[] args)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < 5; ++i)
                {
                    ...
                    model.Update(robot.GetStates().Q, robot.GetStates().DTheta);
                    // Compute gravity vector
                    var g = model.GetGravityForceVector();
                    // Compute mass matrix
                    var M = model.GetMassMatrix();
                    // Compute Jacobian
                    var J = model.GetJacobian("flange");
                ...
                var poseToCheck = (double[])robot.GetStates().TcpPose.Clone();
                poseToCheck[0] += 0.1;
                Console.WriteLine($"Checking reachability of Cartesian pose [{string.Join(", ", poseToCheck)}]");

                var result = model.Reachable(poseToCheck, robot.GetStates().Q, true);
